...
with graph.as_default():
    ...
    with tf.name_scope('hidden1') as scope:
        ...
        biases = tf.Variable(tf.zeros([hn_num]),name='biases')
  ...
  for step in range(num_steps):
    ...
    if (step % 500 == 0):
      ...
      print("Validation accuracy: %.1f%%" % accuracy(
         session.run(train_prediction,feed_dict={tf_train_dataset: data.valid_dataset}), data.valid_labels))

  feed_dict = {tf_train_dataset: data.test_dataset}
